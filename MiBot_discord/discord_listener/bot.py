...
EXCHANGE = 'cartero'
QUEUE = 'weather'
#Creamos el exchange 'mensajero' de tipo 'fanout'
channelMQ.exchange_declare(exchange=EXCHANGE, exchange_type='topic', durable=True)
...
async def cumpleaños(ctx):
    ...
    channelMQ.basic_publish(exchange=EXCHANGE, routing_key="birthday", body=message)
...
@bot.command(name='clima', help='Muestra el clima, temperatura y humedad de una ciudad dada como input. Ejemplo: !clima Puerto-Montt')
async def clima(ctx):
    message = ctx.message.content
    print("mensaje: ",message)
    channelMQ.basic_publish(exchange=EXCHANGE, routing_key=QUEUE, body=message)
...
def writer(bot):
    ...
    channelMQ.queue_bind(exchange=EXCHANGE, queue=queue_name, routing_key="discord_writer")
    ...
    async def write(message):
        for channel in bot.get_all_channels():
            if(channel.name=='general'):
                general = channel.id
                print(f'general: {channel.id}')

        channel = bot.get_channel(general)#913706828502814760
